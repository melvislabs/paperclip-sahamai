...
import { SymbolParamSchema, SymbolsQuerySchema, AnalysisBodySchema, SearchQuerySchema, HistoryQuerySchema, ValidationErrorSchema, SignalWithFreshnessSchema, SignalListResponseSchema, SignalSummaryResponseSchema, HealthResponseSchema, AIAnalysisResultSchema, QuoteResponseSchema, HistoryResponseSchema, NewsResponseSchema, SearchResponseSchema, MetricsResponseSchema, AlertsResponseSchema, SLODashboardResponseSchema, ErrorSchema, SignalSchema } from './auth/schemas.js';
function buildSeedSignals(nowMs) {
    return [
        {
            symbol: 'BBCA',
            action: 'buy',
...
export function buildServer(nowProvider = () => Date.now()) {
    const app = Fastify({
        logger: {
    ...
    const withRequestMetrics = async (route, execute, statusCode = () => 200) => {
    ...
    app.get('/v1/signals/latest/:symbol', {
        ...
        return withRequestMetrics('/v1/signals/latest/:symbol', async () => {
            ...
            refreshFreshness();
            const result = cache.getOrLoad(key, () => store.get(symbol, nowProvider()), nowProvider());
            if (!result) {
    ...
    app.get('/v1/signals/latest', {
        ...
        return withRequestMetrics('/v1/signals/latest', async () => {
            ...
            const data = source.map((symbol) => {
                const key = `signal_registry_head:${symbol}`;
                const item = cache.getOrLoad(key, () => store.get(symbol, nowProvider()), nowProvider());
                if (!item) {
                    return {
                        symbol,
                        stale: true,
    ...
    const analysisCache = new TtlCache(3600000);
    ...
    app.post('/v1/analysis/:symbol', {
        ...
        return withRequestMetrics('/v1/analysis/:symbol', async () => {
            ...
            const cached = analysisCache.getOrLoad(key, () => null, nowProvider());
            if (cached)
                return cached;
            try {
                const result = await analysisService.analyze(symbol, body.priceHistory, body.news ?? []);
                return result;
            }
            catch (error) {
    ...
    const stockApiConfig = buildStockApiConfig({
        provider: process.env.STOCK_API_PROVIDER || 'finnhub',
    ...
    app.get('/v1/stocks/:symbol/history', {
        ...
        return withRequestMetrics('/v1/stocks/:symbol/history', async () => {
            ...
            const symbol = params.symbol.toUpperCase();
            try {
                const result = await stockApiClient.getHistory(symbol, query.interval || '1day', query.from, query.to);
                return result;
            }
            catch (error) {
                return reply.code(502).send({
